...
void BulletCollision(Player &player, std::unique_ptr<Enemy> &enemy) {
  for (Bullet &blt : GameAssets::enemyBullets) {
    if (CheckCollisionRecs(player.rec, blt.rec)) {
      ...
      blt.rec = {GetScreenWidth() + blt.rec.width,
                 GetScreenHeight() + blt.rec.height, 0.0f, 0.0f};
    }
  }
}

void DrawPlayer(Player &player) {
  DrawRectangleLines(player.rec.x, player.rec.y, player.rec.width,
                     player.rec.height, RED);

  DrawTexture(player.shipTextures[player.selectedShipIndex], player.rec.x,
              player.rec.y, WHITE);
}
...
void MovePlayer(Player &player) {
  ...
  if (IsKeyDown(KEY_W) && player.rec.y >= player.rec.height)
    player.movDir.y = -1;
  if (IsKeyDown(KEY_S) &&
      player.rec.y <= (GetScreenHeight() - player.rec.height))
    player.movDir.y += 1;
...
void SpecialAbility(Player &player) {
  if (player.selectedShipIndex == 0 ||
      player.specialMeter < player.specialFullLevel && !player.specialActive) {
    return;
  }
...
bool ExtraShots(Player &player) {
  ...
  player.extraBulletsSpawner.position = {player.rec.x, player.rec.y};
  SpawnBullets(player.extraBulletsSpawner, player.playerBullets,
               GameAssets::bullet, DEG2RAD * 10.0f);
  time += GetFrameTime();
...
void ShootBullet(Player &player, Rectangle bullet) {
  if (IsKeyDown(KEY_SPACE) &&
      (GetTime() - player.lastShot) >= player.shotCooldown) {
    Bullet blt;
    blt.dir = {0, 1};
    blt.sprite = player.bulletSprite;
    blt.rec = bullet; // Reset to Give correct width and height
    blt.rec.x = player.rec.x + ((player.rec.width / 2.0f) * 0.5f) -
                (bullet.width / 2.0f);
  ...
  player.playerBullets.erase(
      std::remove_if(
          player.playerBullets.begin(), player.playerBullets.end(),
          [](const Bullet &blt) {
            return blt.rec.y <
                   -blt.rec.height; // Check if the bullet is off the screen
          }),
...
void BulletsHit(Game &game, std::vector<std::unique_ptr<Enemy>> &enemies) {
  ...
  // NOTE: Have to run loop twice: playerBullets cannot be modified while its
  ...
  for (auto &enemy : enemies) {
    ...
    game.player.playerBullets.erase(
        std::remove_if(
            game.player.playerBullets.begin(), game.player.playerBullets.end(),
            [&enemy](const Bullet &blt) {
              return CheckCollisionRecs(
                  blt.rec,
                  enemy->rec); // Check if the bullet collides with the enemy
            }),
...
void CheckEffects(Player &player) {
  ...
  player.activeEffects.erase(
      std::remove_if(player.activeEffects.begin(), player.activeEffects.end(),
                     [](const auto &p) { return (p.timeLeft <= 0); }),
