...
bool ExtraShots(Player& player)
{
	...
	static float shotTime = 0.0f;
	float shootFor = 5.0f;

	player.extraBulletsSpawner.position = {player.rec.x, player.rec.y};
	time += GetFrameTime();

	if (shotTime >= player.shotCooldown)
	{
		Bullet blt;
		blt.dir = {0.2f, 1};
		blt.sprite = player.bulletSprite;
		blt.rec = GameAssets::bullet; // Reset to Give correct width and height
		blt.rec.x = player.rec.x + ((player.rec.width / 2.0f) * 0.25f) - (GameAssets::bullet.width / 2.0f);
		blt.rec.y = player.rec.y - GameAssets::bullet.height;
		blt.speed = -17;
		player.playerBullets.push_back(blt); // Add the bullet to the vector
		
		blt.dir = {-0.2f, 1};
		blt.rec.x += ((player.rec.width / 2.0f) * 1.2f);
		player.playerBullets.push_back(blt);
		shotTime = 0.0f;
	}
	else
	{
		shotTime += GetFrameTime();
	}
